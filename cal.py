...
root.maxsize(344,505)
root.minsize(344,505)
root.config(bg="#000000")
root.attributes('-alpha',0.97)

#out disaply
out_frame = ctk.CTkFrame(root ,fg_color="#172017" ,border_width=2 ,border_color="#ffffff" ,corner_radius=8 )
out_frame.place(relwidth=1 ,y=5 ,x=5 ,width=-10 ,height=70)
out_display = ctk.CTkLabel(out_frame ,text="0" ,text_font=('arial',17,'bold') , fg_color="#172017" ,corner_radius=6 ,bg_color = "#172017"  ,anchor="e")
out_display.place(relwidth=1 ,y=3,x=6 ,width=-15 ,relheight=1 ,height=-6)
...
def display(input_v):
    ...
    if input_v == ".":
        if len(str(out_display.text)) == 0 :
            out_text = input_v
        else:
            if "." not in str(out_display.text):
                out_text = input_v
            else:
                split_success = False
                for i in maths:
                    try:
                        n,m = str(out_display.text).split(i)
                        split_success = True
                        break
                    except:
                        pass
    ...
    out_display.configure(text=str(out_display.text) + str(out_text))   
...
def remove():
    ...
    if len(str(out_display.text)) > 0:
        if str(out_display.text)[-1] == " ":
            out_display.configure(text=str(out_display.text)[:-3])
        else:
            out_display.configure(text=str(out_display.text)[:-1])
...
def convert(type):
    ...
    if not(ope_code_in) :
        if type == int:
            out_display.configure(text=int(float(out_display.text)))
        else:
            out_display.configure(text=float(out_display.text))
...
bt_frame.place(relwidth=1 ,y=80 ,x=5 ,width=-10 ,relheight=1 ,height=-80)
c_bt = ctk.CTkButton(bt_frame ,text="C",text_font=('arial',11,'bold') ,border_width=2 ,border_color="#707070" ,
...
for i in range(10):
    if column_> 3:
        column_ = 1
        row_ += 1
    globals()['bt_no'+str(i)] = ctk.CTkButton(bt_frame ,text=nums[i] ,text_font=('arial',14,'bold') 
...
float_bt =  ctk.CTkButton(bt_frame ,text="." ,text_font=('arial',14,'bold') 
                                        ,border_width=2 ,border_color="#707070" ,
...
maths = [" / " , " x " ," - " ," + " ]
for i in maths :
    globals()['maths'+str(i)]  = ctk.CTkButton(bt_frame ,text=i ,text_font=('arial',13,'bold') 
                                        ,border_width=2 ,border_color="#707070" ,
...
cal_bt = ctk.CTkButton(bt_frame ,text="=",text_font=('arial',11,'bold') ,border_width=2 ,border_color="#707070" ,
...
rem_bt = ctk.CTkButton(bt_frame ,text="<<<",text_font=('arial',11,'bold') ,border_width=2 ,border_color="#707070" ,
...
con_float = ctk.CTkButton(bt_frame ,text="float",text_font=('arial',11,'bold') ,border_width=2 ,border_color="#707070" ,
...
con_int = ctk.CTkButton(bt_frame ,text="int",text_font=('arial',11,'bold') ,border_width=2 ,border_color="#707070" ,
